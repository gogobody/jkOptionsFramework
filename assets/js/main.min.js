...
window.wp.ajax.post("csf-reset",{unique:c.data("unique"),nonce:c.data("nonce")}).done(function(e){window.location.reload(!0)}).fail(function(e){alert(e.error)})});d.on("click",function(g){g.preventDefault();confirm("\u786e\u5b9a\u5417")&&window.wp.ajax.post("csf-import",{unique:d.data("unique"),nonce:d.data("nonce"),data:a.find(".csf-import-data").val()}).done(function(e){window.location.reload(!0)}).fail(function(e){alert(e.error)})})})};b.fn.csf_field_background=function(){return this.each(function(){b(this).find(".csf--background-image").csf_reload_script()})};
